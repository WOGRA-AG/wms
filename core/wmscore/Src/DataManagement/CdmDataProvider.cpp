...
int CdmDataProvider::XmlExport(QString p_qstrFilename, QString &p_qstrDatabase) const
{
    int iRet = CdmLogging::eDmUnknownDataAccessError;
    QFile qfExport(p_qstrFilename);

    if(qfExport.open(QIODevice::WriteOnly))
    {
        QTextStream qtsXml(&qfExport);
        const CdmScheme* pCdmDatabase = (const_cast<CdmDataProvider*>(this))->FindSchemeByName(p_qstrDatabase);

        if(CHKPTR(pCdmDatabase))
        {
            iRet = pCdmDatabase->XmlExport(qtsXml);
        }
        else
        {
...
int CdmDataProvider::XmlImport(QString &p_qstrFilename)
{
    int iRet = CdmLogging::eDmUnknownDataAccessError;
    QFile qfImport(p_qstrFilename);
    QDomDocument qDomDocument;

    if(qfImport.open(QIODevice::ReadOnly))
    {
        QString qstrError;
        int iLine = 0;
        int iColumn = 0;
        if(qDomDocument.setContent(&qfImport, &qstrError, &iLine, &iColumn))
        {
            iRet = XmlStartImport(qDomDocument);
        }
        else
        {
            ERR("Error occurred by importing xml document.\n Error Message:\n" + qstrError +
                "\nLine: " + QString::number(iLine) + " Column: " + QString::number(iColumn));
        }
    }
...
int CdmDataProvider::XmlStartImport(QDomDocument& p_rqDomDocument)
{
    int iRet = CdmLogging::eDmUnknownDataAccessError;
    QDomElement qdeRoot = p_rqDomDocument.documentElement();

    if(qdeRoot.tagName() == "WMS_DB_ML")
    {
        QDomNode qDomNode = p_rqDomDocument.firstChild();
        while(!qDomNode.isNull())
        {
            QDomElement qDomElement = qDomNode.toElement(); // try to convert the node to an element.
            iRet = CdmScheme::XmlImport(qDomElement);
            qDomNode = qDomNode.nextSibling();
        }
    }
    else
    {
        ERR("No WMS_DB_XML XML file.");
