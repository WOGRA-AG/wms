...
int CdmScheme::XmlExport(QTextStream& p_rqtsXml) const
{
    SYNCHRONIZED_READ;
    int iRet = CdmLogging::eDmUnknownDbError;

    QDomDocument qdDocument("WMS_DB_ML");

    QDomElement qdeRoot = qdDocument.createElement("WMS_DB_ML");
    qdDocument.appendChild(qdeRoot);


    QDomElement qdeTag = qdDocument.createElement("Database");
    qdeRoot.appendChild(qdeTag);

    qdeTag.setAttribute(WMS_ID, QString::number(m_lId));
    qdeTag.setAttribute(WMS_NAME, m_qstrSchemeName);
    qdeTag.setAttribute(WMS_VERSION, QString::number(m_iVersion));

    CdmClassManager* pClassManager = const_cast<CdmScheme*>(this)->GetClassManager();

    if(CHKPTR(pClassManager))
    {
        qdeTag = qdDocument.createElement(WMS_CLASSMANAGER);
        qdeRoot.appendChild(qdeTag);

        iRet = pClassManager->XmlExport(qdeTag);
    }
    else
    {
    ...
    {
        qdeTag = qdDocument.createElement(WMS_CONTAINERMANAGER);
        qdeRoot.appendChild(qdeTag);
        iRet = pContainerManager->XmlExport(qdeTag);
    }
    else
    {
        iRet = EC(eDmInvalidPtr);
    ...
    p_rqtsXml << qdDocument.toString();

    return iRet;
}

int CdmScheme::XmlImport(QDomElement& p_rqdeDatabase)
{
    int iRet = CdmLogging::eDmUnknownDbError;

    if(!p_rqdeDatabase.isNull())
    {
        if(p_rqdeDatabase.tagName() == WMS_SCHEME)
        {
            QString qstrName;
            int iVersion = 0;
            qstrName = p_rqdeDatabase.attribute(WMS_NAME, "");
            iVersion = p_rqdeDatabase.attribute(WMS_VERSION, "0").toInt();
            ...
            {
                CdmScheme* pCdmDatabase = pCdmManager->FindSchemeByName(qstrName);

                if(!pCdmDatabase)
                {
                    INFO("Database does not exist, I have to create it!!!");
                    if(pCdmManager->CreateScheme(qstrName) > 0)
                    {
                        pCdmDatabase = pCdmManager->FindSchemeByName(qstrName);
                    }
                }
                else
                {
                    INFO("Db already exists no new database created!!!");
                }

                if(CHKPTR(pCdmDatabase))
                {
                    pCdmDatabase->SetVersion(iVersion);

                    CdmClassManager* pCdmClassManager = pCdmDatabase->GetClassManager();

                    if(CHKPTR(pCdmClassManager))
                    {
                        iRet = pCdmClassManager->XmlImport(p_rqdeDatabase);
                    }
                    else
                    {
                        iRet = EC(eDmInvalidPtr);
                    ...
                    if(CHKPTR(pContainerManager))
                    {
                        iRet = pContainerManager->XmlImport(p_rqdeDatabase);
                    }
                    else
                    {
                        iRet = EC(eDmInvalidPtr);
