...
CdbCommandGetClass::~CdbCommandGetClass()
{
}

int CdbCommandGetClass::Execute()
{
    ...
    qstrQuery = QString("select ClassId, Keyname, LastChange, DatabaseId, Caption, Comment, PositionSequence, Abstract, CaptionMember, Version, Package_URI, config"
                        " from WMS_CLASS where ClassId = %1")
                        .arg(m_lClassId);
...
long CdbCommandGetClass::LoadMembers(CdmClass* p_pCdmClass)
{
   ...
   if(CHKPTR(p_pCdmClass))
   {
      ...
      // query for reading new id
      qstrQuery = QString("select me.MemberId, me.DataType, me.Must, me.Size, me.ObjectRefClassId, "
                          "me.CounterStart, me.ClassId, me.DefaultValue, me.Keyname, me.Caption, "
                          "me.CreatorId, me.ModifierId, me.Comment, cl.DatabaseId, me.Owner, me.Explicit, "
                          "me.System, me.Access, me.groupId, me.version, me.validation, me.non_persistent, me.Is_Tree, me.config "
                          "from WMS_CLASS_MEMBER me, WMS_DATABASE db, WMS_CLASS cl "
                          "where me.ClassId = %1 and cl.ClassId = me.ClassId and "
                          "db.databaseid = cl.DatabaseId")
                          .arg(p_pCdmClass->GetId());
...
void CdbCommandGetClass::LoadTranslations(CdmMember* p_pCdmMember)
{
   if (CHKPTR(p_pCdmMember))
   {
      QSqlQuery cQuery;
      QString qstrSelect = QString("select Language, Caption, Comment from WMS_CLASS_MEMBER_TRANSLATION where MemberId = %1")
...
void CdbCommandGetClass::LoadGroups(CdmClass* p_pCdmClass)
{
   QSqlQuery cQuery;
   QString qstrSql = QString("select Id, Name, Version, Position, Parent from WMS_CLASS_GROUP where classid = %1")
                     .arg(p_pCdmClass->GetId());
...
void CdbCommandGetClass::LoadGroupTranslations(CdmClassGroup* p_pCdmGroup)
{

   if (CHKPTR(p_pCdmGroup))
   {
      QSqlQuery cQuery;
      QString qstrSelect = QString("select Language, Caption from WMS_CLASS_GROUP_TRANSLATION where GroupId = %1")
...
void CdbCommandGetClass::LoadValidators(CdmClass* p_pCdmClass)
{
   QSqlQuery cQuery;
   QString qstrSql = QString("select Id, Name, Version, Code, ValidationType, Message from WMS_CLASS_VALIDATION where classid = %1")
      .arg(p_pCdmClass->GetId());
   ...
   if (ExecuteQuery(qstrSql, cQuery) > 0)
   {
      ...
      while(cQuery.isValid())
      {
         ...
         p_pCdmClass->AddValidator(pCdmValidator);
...
void CdbCommandGetClass::LoadMethods(CdmClass* p_pCdmClass)
{
   QSqlQuery cQuery;
   QString qstrSql = QString("select Id, Name, Version, Code, MethodType, AccessMode, Parameters, Comment, Icon, Caption, Static from WMS_CLASS_METHOD where classid = %1")
      .arg(p_pCdmClass->GetId());
...
CdmClass *CdbCommandGetClass::GetResult()
{
    return m_rpClass;
}

bool CdbCommandGetClass::CheckValid()
{
    return (m_lClassId > 0);
}
