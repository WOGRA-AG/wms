...
{
    if (ts) {
        ts->writeRawData(str.toLocal8Bit().constData(), str.length());
    ...
    if (ts) {
        while (!ts->atEnd()) {
            ...
            if (len == -1) {
               ...
               result.append(QString::fromLocal8Bit(buffer, len));
            }
        }
    }
...
/*
    ...
    if (::GetDiskFreeSpace((LPCWSTR)partitionPath.utf16(), &bytesPerSector, &sectorsPerCluster, &freeClusters, &totalClusters) == false) {
