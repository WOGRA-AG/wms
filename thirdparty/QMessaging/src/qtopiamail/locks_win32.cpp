...
ProcessMutexPrivate::ProcessMutexPrivate(const QString &path)
    : mutex(0), count(0)
{
    //mutex =::CreateMutex(nullptr, false, reinterpret_cast<const wchar_t*>(path.utf16()));
    if (mutex == 0) {
...
private:
    ...
    int mutexes[MaxConcurrentReaders];
    int mutex;
    int count;
};
    ...
    : mutex(0), count(0)
{
    ...
    for (int i = 0; i < MaxConcurrentReaders; ++i) {
        ...
        //mutexes[i] = ::CreateMutex(nullptr, false, reinterpret_cast<const wchar_t*>(subPath.utf16()));
        if (mutexes[i] == 0) {
