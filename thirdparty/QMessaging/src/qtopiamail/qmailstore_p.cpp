    ...
    while (!requiredTableKeys.isEmpty()) {
        ...
        if (!temporaryTableKeys.contains(arg)) {
            ...
            if (key.second == "INTEGER") {
                int type = 0;
                QVariant qVariant(arg->valueList.first());
                if (qVariant.canConvert<QMailMessageId>()) {
                    type = 1;
                } else if (qVariant.canConvert<QMailFolderId>()) {
                    type = 2;
                } else if (qVariant.canConvert<QMailAccountId>()) {
                    type = 3;
                }
...
{
    return QString::number(reinterpret_cast<long long>(ptr), 16).rightJustified(8, '0');;
}
...
bool QMailStorePrivate::ensureVersionInfo()
{
    if (!database.tables().contains("versioninfo", Qt::CaseInsensitive)) {
        ...
        QString sql("CREATE TABLE versioninfo ("
                    "   tableName NVARCHAR (255) NOT nullptr,"
                    "   versionNum INTEGER NOT nullptr,"
                    "   lastUpdated NVARCHAR(20) NOT nullptr,"
                                                                                   ...
                                                                                   Transaction &t, bool commitOnSuccess)
{
    ...
    if (!folderIdSet.isEmpty()) {
        ...
        QSqlQuery query(simpleQuery("UPDATE mailmessages SET parentfolderid=previousparentfolderid, previousparentfolderid=nullptr",
                                    Key(QMailMessageKey::id(*updatedMessageIds)),
